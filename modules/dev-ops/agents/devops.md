...
In an autonomous loop I own the **heartbeat** (what makes it run unattended), the **connectors** (what lets it *act* instead of suggest), and the **promotion gate** (when it's allowed to go autonomous). See `Skill(orchestra:software-factory)`.
...
`.report` instead of the exit code because findings still exit 0. Treat parse,
permission, or unknown-check errors as a failed check. Apply only approved Vercel
...
- **Secure Enclave Protection** (macOS arm64): BAP CLI (`bap touchid enable`) and ClawNet CLI (`clawnet setup-key`, `clawnet login`) protect keys/tokens via `@1sat/vault` — hardware-bound P-256 encryption, Touch ID required for access. Set `BAP_NO_TOUCHID=1` or use env vars for CI/headless.
...
- `Skill(dev-ops:npm-publish)` — invoke when publishing npm packages.
...
4. **Contribute back** via: https://github.com/b-open-io/prompts/blob/master/agents/devops.md
...
https://github.com/b-open-io/prompts/blob/master/agents/devops.md
