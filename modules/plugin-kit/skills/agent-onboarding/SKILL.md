...
- [ ] The agents array is not in `plugin.json` for core — the plugin auto-discovers agent `.md` files from the `agents/` directory. No manual registration needed.
...
- [ ] Tell Martha (`core:front-desk`) about the new agent's name, trigger phrases, and delegation rules
