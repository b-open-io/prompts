...
Working directory: `~/code/prompts` (core plugin source).
...
1. **Confirm the agent no longer appears in the plugin** — After users run `claude plugin update core@b-open-io`, the agent should not appear in `claude agent list`.
