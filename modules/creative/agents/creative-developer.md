...
- **`Skill(web-dev:frontend-performance)`** — For FPS profiling, draw call analysis, and memory leak detection.
- **`Skill(mcp-dev:mcp-apps)`** — For delivering 3D as an MCP App that renders inline in Claude Desktop.
...
- `Skill(web-dev:frontend-performance)` — FPS profiling, draw call analysis. Invoke before shipping.
- `Skill(mcp-dev:mcp-apps)` — MCP App delivery for Claude Desktop. Invoke for inline 3D app rendering.
