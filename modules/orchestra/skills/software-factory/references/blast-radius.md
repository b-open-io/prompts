...
2. **Harden (watched).** Add stop conditions, work-item admission control, an emergency runaway breaker, the state file, and the never-touch list. Run a few more watched cycles. Measure accept rate and cost-per-accepted-change.
3. **Automate — only if all hold:**
   ...
   - Work-item admission control has been *tested*: it refuses new work without interrupting an admitted item.
