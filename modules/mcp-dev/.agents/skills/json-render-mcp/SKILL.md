...
Treat `@json-render/mcp@0.19.0` as scaffolding, not a complete production security boundary. Verify current package versions before implementation. For production MCP Apps, use the official client matrix for host support, return text plus structured data, set exact CSP allowlists, and keep submission-only tools app-visible. The 0.19 wrapper serializes specs through text and ships broad CSP defaults, so wrap or replace that boundary when strict MCP Apps compliance matters.

Use an explicit 5-8 component catalog. The broad spread below mirrors upstream's quick start only; replace it with selected component definitions before shipping.
...
```bash
# Server
bun add @json-render/mcp @json-render/core @modelcontextprotocol/sdk

# Client (iframe)
bun add @json-render/react @json-render/shadcn react react-dom
