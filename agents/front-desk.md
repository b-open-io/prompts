...
- `twostraws/swiftui-agent-skill@swiftui-pro` — Modern SwiftUI API guardrail (Paul Hudson). Install: `npx skills add https://github.com/twostraws/swiftui-agent-skill --skill swiftui-pro`. Index: https://github.com/twostraws/swift-agent-skills (curated directory; not installed as one skill).
- `twostraws/swift-concurrency-agent-skill@swift-concurrency-pro` — Swift 6 / isolation. Install: `npx skills add https://github.com/twostraws/swift-concurrency-agent-skill --skill swift-concurrency-pro`.
- `twostraws/swift-testing-agent-skill@swift-testing-pro` — Swift Testing over XCTest prose. Install: `npx skills add https://github.com/twostraws/swift-testing-agent-skill --skill swift-testing-pro`.
- `twostraws/swiftdata-agent-skill@swiftdata-pro` — SwiftData when native persistence is in scope. Install: `npx skills add https://github.com/twostraws/swiftdata-agent-skill --skill swiftdata-pro`.
- AvdLee alts (do **not** install alongside twostraws SwiftUI by default; pick one primary — recommended: twostraws): `avdlee/swiftui-agent-skill@swiftui-expert-skill`, `avdlee/swift-concurrency-agent-skill@swift-concurrency`, `avdlee/swift-testing-agent-skill@swift-testing-expert`, `avdlee/core-data-agent-skill@core-data-expert`, `avdlee/xcode-build-optimization-agent-skill` (`--skill xcode-project-analyzer` when optimizing builds). Install pattern: `npx skills add https://github.com/avdlee/<repo> --skill <id>`.
- `openai/plugins` → **`build-ios-apps`** — iOS agent skills (Liquid Glass, UI patterns, App Intents, simulator/debugger, perf). Enable the plugin from https://github.com/openai/plugins; invoke by skill name (`swiftui-ui-patterns`, `swiftui-liquid-glass`, `swiftui-performance-audit`, `swiftui-view-refactor`, `ios-app-intents`, `ios-simulator-browser`, `ios-debugger-agent`, `ios-ettrace-performance`, `ios-memgraph-leaks`). Do not vendor the plugin tree into prompts.
- `openai/plugins` → **`build-macos-apps`** — macOS agent skills (`build-run-debug`, `test-triage`, `swiftui-patterns`, `liquid-glass`, `appkit-interop`, `window-management`, `telemetry`, `swiftpm-macos`, `signing-entitlements`, `packaging-notarization`, `view-refactor`). Enable from https://github.com/openai/plugins. For Vercel Native SDK release, house `native-sdk-macos-release` stays authoritative; OpenAI packaging is supplemental for generic Xcode only.
- Optional Expo↔SwiftUI bridge: `expo-ui-swift-ui` under `openai/plugins` `plugins/expo` (Kira when bridging; does not replace `vercel-react-native-skills`).
- Zabłocki pattern (pointer only): https://merowing.info/posts/stop-getting-average-code-from-your-llm/ — teach product repos `AGENTS.md` + `ai-rules/rule-loading.md` + `general.md`; do **not** paste course rule bodies into prompts.
- AppCreator / Super Easy Apps build-loop **pattern** → use house `apple-xcode-build-loop` (Makefile, xcbeautify, warnings-as-errors, destination policy). Do **not** vendor the closed kit or paste private skill text.
