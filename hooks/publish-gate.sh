...
on_unexpected_error() {
  ...
  if [[ "${_PUBLISH_GATE_ACTIVE:-false}" == "true" ]]; then
    ...
    printf '%s\n' '{"hookSpecificOutput":{"permissionDecision":"deny"},"systemMessage":"PUBLISH GATE: internal error while evaluating publish approval. Safe publish path: run the bopen-plugin-dev:publish-request skill, check LINEAR_API_KEY and network access, have a human move the Linear ticket to Approved, then retry through the gate."}' >&2
    exit 2
  fi
  exit 0
}
...
publish_gate_main() {
  ...
  if [[ -z "${LINEAR_API_KEY:-}" ]]; then
    if [[ "$IS_ON_CHAIN" == "true" ]]; then
      deny_permission "PUBLISH GATE (on-chain): LINEAR_API_KEY is not set. clawnet publish --on-chain broadcasts an irreversible BSV transaction to mainnet. Safe publish path: run the bopen-plugin-dev:publish-request skill, obtain an Approved Linear ticket, have its approver comment exactly 'irreversible acknowledged', set LINEAR_API_KEY from a human terminal, then retry through the gate."
    else
  ...
  if printf '%s' "$LINEAR_RESPONSE" | jq -e '.errors and (.errors | length > 0)' >/dev/null 2>&1; then
    ...
    deny_permission "PUBLISH GATE: Linear GraphQL error: ${ERR_MSG}. Safe publish path: run the bopen-plugin-dev:publish-request skill, fix the Linear request or credentials, have a human confirm the ticket is Approved, then retry through the gate."
  fi
  ...
  if [[ "$IS_ON_CHAIN" == "true" && -n "$TICKET_ID" ]]; then
    ...
    if [[ $COMMENTS_STATUS -ne 0 || -z "$COMMENTS_RESPONSE" ]]; then
      deny_permission "PUBLISH GATE (on-chain): failed to load comments for ticket ${TICKET_IDENTIFIER}. Cannot verify irreversible acknowledgment. Safe publish path: keep the release in bopen-plugin-dev:publish-request, restore Linear access, verify the approver's exact 'irreversible acknowledged' comment, then retry through the gate."
    fi
    ...
    if [[ "$HAS_ACK" != "true" ]]; then
      deny_permission "PUBLISH GATE (on-chain): Linear ticket ${TICKET_IDENTIFIER} (${TICKET_TITLE}) is Approved, but no comment containing \"irreversible acknowledged\" was found. Safe publish path: have the approver add that exact phrase to the bopen-plugin-dev:publish-request ticket, then retry through the gate."
    fi
  fi
  ...
  STOP_REASON="${STOP_REASON}. Safe publish path: run the bopen-plugin-dev:publish-request skill to prepare the release plan in Linear, have a human review it and move the ticket to Approved, then retry through the gate."
