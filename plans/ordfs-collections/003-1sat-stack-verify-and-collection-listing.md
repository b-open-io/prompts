...
- **Authorization identity (ONE open parameter — confirm before building the signer-match):** the "root signer" a member must match is EITHER (A) the root's **origin** author identity (fixed) OR (B) the **current `:-1` owner** (follows the transfer chain). Metadata-update authority is inherently B. Implement the membership signer-match behind this parameter; default to (A) origin-pinned unless the operator selects (B). Do not hardcode one silently. A `ref=ordfs`, `ord-fs/json`, `text/uri-list`, `base64Content`, OR `bsv-21 deploy` item are all the same at the MAP layer, so the listing admits them identically and must not inspect the leaf. Membership is anchored to the item's origin/outpoint — do NOT filter `unspent=true` (a deploy output is spent on distribution; `Search` defaults `unspent=false` at `pkg/txo/routes.go:279`, keep it).
