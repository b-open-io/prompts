...
- **The fix:** in `ParseMAP`, unwrap `subTypeData` and emit `map:subType:collectionItem` + `map:collectionId:<id>` events, so `GET /search?key=ev:map:collectionId:<id>` returns members. Then add the read endpoint that lists collectionItem outpoints for a `collectionId` + minimal MAP (mintNumber, rarityLabel) for UI.
- **Membership invariant (settled — Q2):** membership keys ONLY on MAP (`collectionId` + `subType:collectionItem` + valid AIP), NEVER on the inscription body or content-type. A `ref=ordfs`, `ord-fs/json`, `text/uri-list`, `base64Content`, OR `bsv-21 deploy` item are all the same at the MAP layer, so the listing admits them identically and must not inspect the leaf. Membership is anchored to the item's origin/outpoint — do NOT filter `unspent=true` (a deploy output is spent on distribution; `Search` defaults `unspent=false` at `pkg/txo/routes.go:279`, keep it).
...
- **MAJOR — no balance×membership join.** Membership is anchored to the deploy outpoint (`tokenId = deploy outpoint`, `pkg/parse/bsv21.go:42-47`); holder UTXOs carry `tokenId` but no collection MAP. "Show my collection items" needs: holder balance (bsv21 lookup) → `tokenId` → deploy outpoint → deploy MAP (general store) → `subTypeData.collectionId`. Add a `tokenId → collection` resolver (cache deploy MAP by tokenId) so clients can compute the join. The SDK's `Bsv21TokenData`/`TokenDetailResponse` (1sat-sdk `packages/types/src/services.ts:287-304`) also carries no MAP field, so this resolver is what closes that gap server-side.
- Depends on Part B's `map:collectionId:<id>` event emission — that is what makes a token member (or any member) enumerable at all.
...
4. **(Conditional) Membership indexing + listing (Part B):** only if Step 3 says yes. First emit `map:subType:collectionItem` + `map:collectionId:<id>` events in `ParseMAP` (`pkg/parse/bitcom.go:74-76`), unwrapping `subTypeData`. Then add the listing route + handler + a `go test` covering one collection with THREE members — a `base64Content` NFT, a `ref=ordfs` NFT, and a `bsv-20` token deploy — asserting ALL THREE are returned (proves membership ignores the leaf and admits token deploys). **Verify**: `go test ./...` passes; `GET /search?key=ev:map:collectionId:<id>` returns all three.
...
6. **Token-member join (Part E):** verify (operationally) the general indexer ingests deploy txs; add the `tokenId → collection` resolver so a holder's balance resolves to collection membership. Add a `go test`: a `bsv-20` deploy carrying `collectionItem` MAP is enumerable via the Part-B listing AND resolvable from `tokenId`. **Verify**: `go test ./...` passes; the resolver returns the `collectionId` for a token deploy's `tokenId`.
...
- [ ] If Part B built: `ParseMAP` emits `map:collectionId:<id>`; endpoint returns a `base64Content`, a `ref=ordfs`, AND a `bsv-20` token-deploy member for one `collectionId`
- [ ] Part D: `ref=ordfs` resolves under real MIME + `content-ref.md` headers; `text/uri-list` items follow their URI; `go test ./...` passes
- [ ] Part E: general indexer ingests deploy txs (verified); `tokenId → collection` resolver returns the `collectionId` for a token deploy
