...
- **The fix:** after all parsers have run, require `subType=collectionItem`, parse `subTypeData` strictly, and normalize `collectionId` to an absolute outpoint (`_N` resolves against the current txid). Treat `map:subType:collectionItem` + `map:collectionId:<absolute-id>` as candidate discovery keys. The authoritative listing then loads the referenced root, requires `subType=collection`, requires a valid authorship signature (SIGMA preferred — input-bound/replay-resistant — or legacy AIP; the parser already emits `signer:` events for both at `pkg/parse/bitcom.go:104,150`), and requires signer-set intersection between root and member. (`ParseMAP` runs before `ParseAIP`, and root lookup is external, so `ParseMAP` alone cannot enforce these invariants.) Raw MAP remains queryable as parsed data, but an unsigned, invalidly signed, foreign-signed, malformed, or non-collection-root record is never returned as authoritative membership.
- **Membership invariant (settled — Q2):** membership keys ONLY on MAP (`collectionId` + `subType:collectionItem` + a valid authorship signature — SIGMA preferred, AIP accepted for legacy — whose signer matches the collection root), NEVER on the inscription body or content-type.
- **Root identity vs metadata (settled):** `collectionId` is the root's **origin** outpoint (stable). The collection root is an owned 1-sat ordinal whose METADATA is updatable by spend + re-inscribe; resolve current metadata at `{root}:-1` (`forwardCrawl`, `pkg/ordfs/ordfs.go:572`), but match membership on the origin `collectionId`, not the tip.
- **Authorization identity (DECIDED — option B, follows the current owner):** the "root signer" a member must match is the key that controls the root ordinal, which moves with the transfer/spend chain (NOT pinned to the origin author). Transferring the root delegates authority. **Validate point-in-time:** a member is authoritative iff its authorship signer matched the root's controlling key *as of the member's inscription height* — so a root sale moves authority forward for future members/metadata WITHOUT retroactively invalidating members already minted under the prior owner. Concretely: resolve the root's spend chain (`forwardCrawl`/`loadSpend`, `pkg/ordfs/ordfs.go:572`, `:167-169`) to determine the controlling key at the member's height, and compare the member's `signer:` against it. Add tests for: member minted under owner-1 stays valid after the root transfers to owner-2; a member minted under owner-2 matching owner-2 is valid; a member whose signer matches neither is rejected. A `ref=ordfs`, `ord-fs/json`, `text/uri-list`, `base64Content`, OR `bsv-21 deploy` item are all the same at the MAP layer, so the listing admits them identically and must not inspect the leaf. Membership is anchored to the item's origin/outpoint — do NOT filter `unspent=true` (a deploy output is spent on distribution; `Search` defaults `unspent=false` at `pkg/txo/routes.go:279`, keep it).
...
**Part D — resolver reference branches (mostly resolved upstream; verify + optional uri-list):**
- **`ref=ordfs` — DO NOT add a resolve branch.** The maintainer implemented then REMOVED ref=ordfs upstream (`3e89a97 ordfs: remove content-ref (ref=ordfs); keep directory defaults`, deleted `pkg/ordfs/ref.go`) — matching the round-2 decision to drop it. The SDK does not produce it. Confirm `grep -rn "ref=ordfs\|content-ref" pkg/ordfs` on the fetched HEAD → gone; do not reintroduce.
- **`text/uri-list` ACCEPT branch — OPTIONAL (interop only).** Not implemented upstream (`git grep uri-list pkg/` → none). Since the SDK produces `ord-fs/json`, this is a nice-to-have for interop with external tools that emit uri-list, not a gate. If built: one exact-match branch reusing the pointer resolver; the resolver never emits uri-list. Defer unless an interop need is concrete.
...
**Part F — generic default-entry (`.`) for `ord-fs/json` directories — ALREADY MERGED UPSTREAM (verify only):**
- **Status: DONE.** Landed in `1sat-stack` `3e89a97` (2026-07-15): `dirDefaultDot="."` takes precedence over `index.html` via `pickDefaultDirectoryKey` (`routes.go:146-153`), served in place (not redirected); `index.html` kept only for HTML sites; an informative "directory has no default entry (. or index.html)" error replaces the old blind 302→404. This is exactly what the round-2 bench specified, implemented by the maintainer. **Do not re-implement.** Just confirm on the fetched HEAD that the behavior matches (a `{ ".": "<img>" }` directory served at the bare outpoint returns the image in place) and that the SPA fallback uses `index.html` only, not `.` (`routes.go:194-197`).
- **Historical (pre-merge) context, for reference:** the old behavior 302-redirected an empty path to `index.html` and 404'd when a directory's primary content was a non-HTML image — the knock this fixed.
