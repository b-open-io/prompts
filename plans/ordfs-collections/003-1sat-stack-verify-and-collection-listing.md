...
- **Membership invariant (settled — Q2):** membership keys ONLY on MAP (`collectionId` + `subType:collectionItem` + a valid authorship signature — SIGMA preferred, AIP accepted for legacy — whose signer matches the collection root), NEVER on the inscription body or content-type. A `ref=ordfs`, `ord-fs/json`, `text/uri-list`, `base64Content`, OR `bsv-21 deploy` item are all the same at the MAP layer, so the listing admits them identically and must not inspect the leaf. Membership is anchored to the item's origin/outpoint — do NOT filter `unspent=true` (a deploy output is spent on distribution; `Search` defaults `unspent=false` at `pkg/txo/routes.go:279`, keep it).
