...
> **ARCHIVED — DO NOT EXECUTE.** This July 2026 design predates the shipped
> `1sat-stack/pkg/collection` contract and preserves rejected assumptions about
> AIP, root-signer matching, ownership-following authority, and BSV21 coupling.
> Use `Skill(1sat:collections)` and
> `references/development/1sat-collections.md` for current behavior.
...
3. **The collection root — SETTLED.** The root is a classic `image/*` (or `ord-fs/json` presentation dir) MAP `subType:collection` — NOT a container that enumerates/owns members (items are owned by different wallets and trade individually). `collectionId` = the root's **origin** outpoint (stable identity, survives re-inscription); collection **metadata is updatable** by the root's owner (spend + re-inscribe; readers resolve `{root}:-1` — supported by `forwardCrawl`, `pkg/ordfs/ordfs.go:572`). Membership is **open + query-discovered**. **Authorization follows the current owner (option B, decided):** authority for both metadata-update and membership tracks whoever currently controls the root ordinal — transferring the root delegates authority (a feature: send it to another wallet for any purpose). **Validate membership point-in-time:** a member is authoritative if its signer matched the root's controlling key *as of the member's inscription height*, so a later sale moves authority forward without retroactively orphaning members already minted. Confirm this temporal rule during the indexer PR (plan 003 Part B).
