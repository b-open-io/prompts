...
3. **The collection root — SETTLED as owned + `:-1`-updatable; ONE open fork (authorization identity).** The root is a classic `image/*` (or `ord-fs/json` presentation dir) MAP `subType:collection` — NOT a container that enumerates/owns members (items are owned by different wallets and trade individually). `collectionId` = the root's **origin** outpoint (stable identity, survives re-inscription); collection **metadata is updatable** by the root's owner (spend + re-inscribe; readers resolve `{root}:-1` — supported by `forwardCrawl`, `pkg/ordfs/ordfs.go:572`). Membership is **open + query-discovered**. The one open call: does a member's required signer match the root's **origin author** (A — stable, no delegation) or the **current `:-1` owner** (B — delegable, but membership authority moves on sale)? Metadata-update authority is inherently B (only the UTXO owner can spend); this fork is about *membership* authorization. Plan 003's signer-match must be parameterized on this choice.
