...
MintFlow's item minting currently dedups the image via a **non-standard `x-ordfs=alias`** trick that no OrdFS server in these repos resolves (`components/mint/steps/step-tier-items.tsx` builds 2-byte `_0` aliases with content-type `…; x-ordfs=alias`). It works only if the production resolver happens to support that convention — unproven. Part A replaces it with an **`ord-fs/json` item using a `.` default-entry** — the settled first-class form (re-run 2026-07-15): the item is a directory `{ ".": "<imageOutpoint>" }` whose content-type honestly signals resolution and whose `.` entry the resolver serves in place (plan 003 Part F). `ref=ordfs` was DROPPED (unanimous bench — it fails unsafe, declaring `image/png` over a pointer body); `text/uri-list` is accepted by the resolver but never produced here. Part B adds a **Mint on Buy** deployment mode so items are minted one-per-purchase (dynamic supply, no pre-minted inventory) instead of pre-minting the whole declared quantity.
...
- `components/mint/steps/step-tier-items.tsx` — remove the `x-ordfs=alias` construction; instead, inscribe the tier image ONCE (as today, output 0), then mint each item as an `ord-fs/json` inscription whose content is a directory `{ ".": "<imageOutpoint>" }` referencing the shared image. Prefer the SDK's new `mintCollectionItem({ ref })` (plan 001) if wired; otherwise build the `ord-fs/json` output inline (`content-type: ord-fs/json`, body = the directory JSON with a `.` entry — use the relative `_0` form only within the same tx, else the absolute `txid_0`).
...
- `components/mint/steps/step-review.tsx` + supporting lib — add `hostingOption: "mint-on-buy"`. Persist the collection config; on a purchase event, mint ONE item (an `ord-fs/json` `{ ".": "<imageOutpoint>" }` reference to the shared image) to the buyer. Requires a server route + a persistence store for the config and per-sale mint. Treat the declared `totalQuantity` as a CAP, not a pre-mint count.
...
1. Replace the alias construction in `step-tier-items.tsx` with a reference build: image inscribed once at output 0; items reference it. **Verify**: `bunx tsc --noEmit` exit 0; inspect the built outputs in a unit/manual check — item outputs carry `ord-fs/json` content (a directory with a `.` entry pointing at the image outpoint) and the shared image is inscribed once.
...
- Part A: a unit test (or a scripted dry-run) asserting the batch produces one image inscription + N `ord-fs/json` reference outputs (each a directory with a `.` entry pointing at the image), all basketed; and no `x-ordfs=alias` string remains.
...
- [ ] Part A: `grep -rn "x-ordfs=alias" .` → no matches; items mint as `ord-fs/json` references (directory with a `.` entry); image inscribed once
