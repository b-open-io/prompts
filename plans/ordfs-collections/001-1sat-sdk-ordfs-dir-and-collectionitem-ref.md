...
- `packages/actions/src/tokens/index.ts` — **generic extended-data, NOT a collectionItem-specific param (per shruggr's PR #21 review, 2026-07-15).** Add `map?: Record<string, string>` and `signWithBAP?: boolean` to `deployBsv21Mint` / `deployBsv21Auth`, mirroring `InscribeRequest` (`inscriptions/index.ts:46-49`) verbatim. BSV21 carries arbitrary extended MAP data plus an optional SIGMA signature and knows nothing about collections. shruggr: *"This should be generic MAP data, like inscribeOrdinal does… we shouldn't extend BSV21 specifically for collection, but rather for any type of extended data. Also probably need to extend it to allow sigma signature."* Grounded: the template options expose `parent`/`scriptSuffix` (`templates/src/bsv21/bsv21.ts`), `BSV21.lock(lockingScript)` uses its argument as the suffix, so the action composes P2PKH + (map ? MAP : none) before `lock`, then appends SIGMA (via `applySigma`) when `signWithBAP`. Shipped precedent: P2PKH + non-image inscription + MAP + AIP in one output (`registry/package-tx.ts:53-243`). Deploy actions today build a single output with NO MAP/sign path (`tokens/index.ts:1182-1187`, `1309-1311`). Do not change the `bsv-20` JSON.
- `packages/actions/src/collections/index.ts` — the collectionItem MAP assembly moves HERE (out of `tokens/`): `buildCollectionItemMap` / `collectionIdToParentBytes` build the MAP record, and a collection-layer helper calls `deployBsv21Mint({ …, map: <collectionItem MAP>, signWithBAP: true })` for a token member. Collection actions own collection-specific fields; the token action stays generic.
- **Match Dave's merged stack contract** (`1sat-stack pkg/collection/mapdata.go`, OPL-2993): the emitted MAP is top-level `type`, `subType` (`collection` | `collectionItem`), `name`, `subTypeData`; `collectionId` / `mintNumber` / `rank` live INSIDE the `subTypeData` JSON string (never as flat MAP keys), and SIGMA authorship is validated by `FirstValidSigma`. `ref=ordfs` is gone (Dave removed it, matching round 2). **Plan 003 (1sat-stack collection listing) is SUPERSEDED** — Dave shipped `pkg/collection` (discovery topic `tm_1sat_collection`, per-collection `tm_col_{id}`, mint-only admission, `map:subType`/`map:collectionId` parse events). Our SDK's job is to emit exactly what his parser reads.
- **Fix the pre-existing signature gap — with SIGMA, not AIP:** `mintCollection`/`mintCollectionItem` apply no authorship signature at all, though the spec requires collection/item authorship signing (`collections.md`, `collectionitem-subtype.md`). Use **SIGMA** (the SIGMA protocol prefix commits to a specific transaction input, so the signature is replay-resistant — it cannot be lifted onto another item/tx, which is what makes membership forgery-resistant). This matches the 1-sat-ordinal convention (line 34). **The primitive already exists:** `applySigma(...)` in `packages/actions/src/signing/sigma.ts` (exported from `signing/index.ts`) builds a `SIGMA BSM <address> <compactSig> <vin>` suffix committing to an input outpoint (`getInputHash(txid, vout)`) — no new signing primitive needed, just wire it into the collection/item/token-deploy builders. Apply SIGMA authorship to the collection root and all items — including token-deploy members — using the same resolved identity so the signer matches the root and membership is provable. Legacy AIP remains accepted on read (the stack emits `signer:` for both), but new mints sign with SIGMA.
