...
import {
  GAME_INPUT_BINDING_BY_ID,
  GAME_INPUT_BINDINGS,
  type GameInputActionId,
} from "./bindings";
...
function createInitialActions(): Record<
  GameInputActionId,
...
function hasBlockingDialog(): boolean {
  ...
  return Array.from(dialogs).some(
    (dialog) => !dialog.closest("[data-game-shell-root]"),
  );
}
...
export function GameInputProvider({ children }: { children: React.ReactNode }) {
  ...
  const publishAction = useCallback(
    (id: GameInputActionId, next: GameInputActionState) => {
