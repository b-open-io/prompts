...
function paintButton(
	...
	for (let y = 0; y < rows; y++) {
		const density =
			variant === "gradient" ? 0.25 + 0.75 * ((y + 0.5) / rows) : variant === "dotted" ? 0.5 : 0.75
...
export function DitherButton({
	...
	// biome-ignore lint/correctness/useExhaustiveDependencies: replayToken/bloom are intentional redraw triggers (upstream dither-kit behavior)
	useEffect(() => {
