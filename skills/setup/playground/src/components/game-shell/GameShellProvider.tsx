...
import { GameInputProvider, useGameInput, useHoldAction } from "@/lib/game-input"
...
export function openGameScreen(id: GameScreenId) {
	window.dispatchEvent(new CustomEvent<GameScreenId>(OPEN_EVENT, { detail: id }))
}
...
function GameShellPortal() {
	...
	const requestClose = useCallback(() => {
		...
		timerRefs.current.push(
			window.setTimeout(finishClose, reducedMotion ? REDUCED_FADE_MS : CLOSE_MS),
		)
	}, [activeScreen, beginPhase, clearTimers, finishClose, phase, play, reducedMotion])
	...
	}, [activeScreen, beginPhase, clearTimers, openControls.pressed, phase, reducedMotion])
	...
	const showScreen = Boolean(activeScreen) && (phase === "screen" || phase === "close")
	const ActiveScreen = activeScreen ? GAME_SCREEN_REGISTRY[activeScreen].component : null
			...
			{showScreen && ActiveScreen && activeScreen && (
				<div className={styles.screenStage}>
					...
					<div
						...
						tabIndex={-1}
					>
						<ActiveScreen closeButtonRef={closeButtonRef} inputMode={mode} onClose={requestClose} />
					</div>
				</div>
			)}
		</div>,
