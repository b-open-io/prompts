...
function createSquares(width: number, height: number): Square[] {
  ...
  return Array.from({ length: count }, (_, index) => {
    ...
    const offX =
      side === 0 ? -margin : side === 1 ? width + margin : random() * width;
    const offY =
      side === 2 ? -margin : side === 3 ? height + margin : random() * height;
    return {
      bandX,
      bandOffsetY,
      offX,
...
function drawSquares(
  ...
  for (const square of squares) {
    ...
    const targetY =
      bandCenterY(square.bandX, width, height) + square.bandOffsetY;
    const x = square.offX + (square.bandX - square.offX) * progress;
    const y =
      square.offY +
      (targetY - square.offY) * progress +
      Math.sin(progress * Math.PI) * square.arc;
    ...
    const draw = (now: number) => {
      ...
      if (!reducedMotion) {
        if (phase === "charge") {
          ...
          visualChargeRef.current +=
            (target - visualChargeRef.current) * Math.min(1, delta * rate);
          ...
          const progress = easeInOutCubic(
            clamp01((now - phaseStartedAt) / GROW_MS),
          );
          const cover = height + Math.abs(BAND_SLOPE * width);
          const rawHalfHeight =
            BAND_HEIGHT / 2 + (cover - BAND_HEIGHT / 2) * progress;
          ...
          if (elapsed < CLOSE_COLLAPSE_MS) {
            const progress = easeInOutCubic(
              clamp01(elapsed / CLOSE_COLLAPSE_MS),
            );
            const cover = height + Math.abs(BAND_SLOPE * width);
            ...
            drawBand(
              ...
              width,
              height,
              Math.ceil(rawHalfHeight / 8) * 8,
              primary,
            );
          } else {
            const shatter = clamp01(
              (elapsed - CLOSE_COLLAPSE_MS) / CLOSE_SHATTER_MS,
            );
            drawBand(
              context,
              width,
      ...
      const chargeIsSettling =
        phase === "charge" &&
        Math.abs(targetChargeRef.current - visualChargeRef.current) > 0.002;
