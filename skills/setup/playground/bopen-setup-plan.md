# bOpen Setup Plan

Generated: 2026-07-13T19:36:46.804Z
Target runtime: claude

This plan was produced by the bopen setup installer from user selections; execute top to bottom; every step lists its verification command.

Execute via Skill(orchestra:coordinator) where available; after executing, re-run the installer's Refresh to confirm state.
